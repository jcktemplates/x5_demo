...
(function($)
{
	...
	Redactor.fn = $.Redactor.prototype = {
		...
		destroy: function()
		{
            ...
            // Remove tooltips on destroy -KJI
			...
			else
			{
                ...
                $elem[0].firstChild.nodeValue = html;
				$elem.removeClass('redactor_editor').removeClass('redactor_editor_wym').removeAttr('contenteditable').show();
			}
            ...
            if ($.isFunction(this.opts.afterDestroyCallback))
			{
				$.proxy(this.opts.afterDestroyCallback, this)();
			}
		},
		...
		buildAfter: function()
		{
			...
			// focus
			if (this.opts.focus === 'allButIE' && !this.browser('msie')) setTimeout($.proxy(this.focus, this), 100);
			else if (this.opts.focus === true) setTimeout($.proxy(this.focus, this), 100);
		...
		dropdownBuild: function($dropdown, dropdownObject)
		{
			$.each(dropdownObject, $.proxy(function(btnName, btnObject)
			{
				...
				else
				{
					...
					{
                        // Click on toolbar bubbles body click. To prevent this, we set
                        // a preventing variable -KJI
                        dontClickBody = true;
                        setTimeout(function() {
                            dontClickBody = false;
                        }, 300);
		...
		dropdownShow: function(e, key)
		{
			...
			else
			{
                ...
                // IE 8 fix for making dropdown on top of redactor editable area
                if (this.oldIE()) $('.redactor_box').css('filter', 'alpha(opacity=99)');
			}
        ...
        buttonAddAfterNextSeparator: function(afterkey, key, title, callback, dropdown)
		{
			if (!this.opts.toolbar) return;
			var btn = this.buttonBuild(key, { title: title, callback: callback, dropdown: dropdown });
			var $btn = this.buttonGet(afterkey);
			var $btnNextSeparator = $btn.parent().nextAll('li.redactor_separator').first();
            
			if ($btn.size() !== 0) $btnNextSeparator.after($('<li>').append(btn));
			else this.$toolbar.append($('<li>').append(btn));
		},
		...
		execCommand: function(cmd, param, sync)
		{
			...
			if (cmd === 'insertunorderedlist' || cmd === 'insertorderedlist')
			{
				...
				if (remove)
				{
                    ...
                    // The following stock redactor line didn't take into consideration
                    // that the list tag could contain whitespaces only. I changed this -KJI
					//this.$editor.find(listTag + ':empty').remove();
                    this.$editor.find(listTag).each(function() {
                        if ($(this).text().trim() === '') {
                            $(this).remove();
                        }
                    });
				...
				{
                    // IE crashes when inserting e.g. an ordered list on special content (spans with linebreaks)
                    // Therefore, before execCommand we prevert <br>'s -KJI
                    /*html = this.$editor.html();
                    html = html.replace(/<br>/gi, '[br]');
                    this.$editor.html(html);*/
                    // Update: this has become unnessecary because the X5 HTML is now cleaned up prior to editing
					...
					if ($list.length)
					{
						if ((this.browser('msie') || this.browser('mozilla')) && parent.tagName !== 'LI' && parent.tagName !== 'UL' && parent.tagName !== 'OL')
						{
		...
		inlineRemoveStyleRespectBordersOfElems: function(rule, newValue, removeClassesMatch)
		{
			//
			// will get all elements in range and cut them when the range intersects
			//
			this.selectionSave();
			if (!newValue) {newValue=''};
            var nodes = this.getNodes();

			blockElements = 'p, div, h1, h2, h3, h4, h5, h6, blockquote, dl, ul, fieldset, form, hr, ol, li, pre, table';

			if (nodes.length === 1){
				// if there is one element only, it's a text element
				// and redactor has added a span around it, so we just decolor that span
				if (nodes[0].nodeType !== 1) {
					nodes.push(nodes[0].parentElement);
				};
			}

			// find elements that contains markers
            if (!this.opts.rangy) {
                var startMarker = this.$editor.find("[id^=selection-marker-1]"),
                    endMarker   = this.$editor.find("[id^=selection-marker-2]");
            } else {
                var startMarker = this.$editor.find(".rangySelectionBoundary").first(),
                    endMarker   = this.$editor.find(".rangySelectionBoundary").last();
            }

			// split that element where the marker is
			var startParent = startMarker.parent(), endParent = endMarker.parent();

            var closestBlockStart   = startParent.closest(blockElements)[0],
                closestBlockEnd     = endParent.closest(blockElements)[0],
                closestRootBlockEnd;

            // Block elements don't have any style/classes applied.
            // There are 2 cases for creating edges:

            // 1) If the closest ancestor block element for start and end marker 
            //    is the same (<p>aa[start]aa[end]</p>):
            //    Split (inline) elements inside up to this block element            
            if (closestBlockStart === closestBlockEnd) {

                var elementToSplitUpTo = startParent.closest(blockElements)[0];

                nodes = this.inlineRemoveStyleCreateEdges(startParent[0],
                    startMarker[0], 
                    nodes,
                    elementToSplitUpTo,
                    true);

                // Find endMarker parent again - the above function might have
                // moved the endMarker to a new element
                if (!this.opts.rangy) {
                    endMarker   = this.$editor.find("[id^=selection-marker-2]");
                } else {
                    endMarker   = this.$editor.find(".rangySelectionBoundary").last();
                ...
                endParent   = endMarker.parent();

                nodes = this.inlineRemoveStyleCreateEdges(endParent[0],
                    endMarker[0], 
                    nodes,
                    elementToSplitUpTo,
                    false);	

                this.inlineRemoveStyleRemoveAndSetRule(nodes, rule, newValue, removeClassesMatch);

            // 2) The closest ancestor block element for start and end marker 
            //    is NOT the same (<p>aa[start]aa</p><p>aaaa</p><p>aa[end]aa</p>):
            //    Run split function (create edges) for each block element
            } else {

                //
                // (middle) Remove style and set rule for elements in between start and end block elements
                //
                if ($(closestBlockEnd).parent().is(this.$editor)) {
                    closestRootBlockEnd = closestBlockEnd;
                } else {
                    closestRootBlockEnd = $(closestBlockEnd).parentsUntil(this.$editor).last();
                }

                this.inlineRemoveStyleRemoveAndSetRule($(closestBlockStart).nextUntil($(closestRootBlockEnd)), rule, newValue, removeClassesMatch);


                //
                // (start) Take care of closest ancestor block to start marker
                //
                
                // Get nodes in start block element (excluding the block element itself)
                startBlockNodes = $.makeArray($(nodes).filter(function() {
                    return $(this).closest(closestBlockStart).length && this !== closestBlockStart;
                }));

                var elementToSplitUpTo = closestBlockStart;

                startBlockNodes = this.inlineRemoveStyleCreateEdges(startParent[0],
                    startMarker[0], 
                    startBlockNodes,
                    elementToSplitUpTo,
                    true);

                this.inlineRemoveStyleRemoveAndSetRule(startBlockNodes, rule, newValue, removeClassesMatch);


                //
                // (end) Take care of closest ancestor block to end marker
                //

                // Get nodes in end block element (excluding the block element itself)
                endBlockNodes = $.makeArray($(nodes).filter(function() {
                    return $(this).closest(closestBlockEnd).length && this !== closestBlockEnd;
                }));

                var elementToSplitUpTo = closestBlockEnd;

                endBlockNodes = this.inlineRemoveStyleCreateEdges(endParent[0],
                    endMarker[0], 
                    endBlockNodes,
                    elementToSplitUpTo,
                    false);

                this.inlineRemoveStyleRemoveAndSetRule(endBlockNodes, rule, newValue, removeClassesMatch);
        ...
        inlineRemoveStyleRemoveAndSetRule: function (nodes, rule, newValue, removeClassesMatch)
        {
            // Set CSS
			$(nodes).find('*').andSelf()
			...
			// X5 sets various attribs in CSS, we don't want that to remain

            $(nodes).each(function() {
                $(this).find('span').andSelf().each(function() {

                    // If not text node, remove classes
                    if (this.nodeType !== 3) {
                        $(this).removeClass(function(index, classes) {
                            var matches = classes.match(removeClassesMatch); // e.g. /\b(ff|fs|cb|cf)\d+\b/gi
                            if (matches) {
                                return matches.join(' ');
                            } else {
                                return '';
        ...
        findCommonAncestor: function(element1, element2)
        {
            var element1Ancestors = $.makeArray($(element1).parentsUntil(this.$editor.parent())),
                returnVal = false;
        
            $(element2).parentsUntil(this.$editor.parent()).each(function() {
                // Common element found
                if (element1Ancestors.indexOf(this) > -1) { 
                    returnVal = this;
                    return false;
                }
            });
		...
		inlineRemoveStyleCreateEdges: function(parent,marker,nodes,commonAncestorContainer,isStartMarker)
        {
            var oldParent;
            do {

                // Don't split editor div
                if (parent === this.$editor[0]) {
                    break;
                }
                
				var parentClone = $(parent).clone()[0];

				// Children got copied over -- remove those
				$(parentClone).text('');

				// Find the marker position in this node
				var indexOfStart = $.inArray(marker,parent.childNodes);

				if (isStartMarker) {
					this.moveNodesLeft(parent, parentClone,indexOfStart);
				} else {
					this.moveNodesRight(parent,parentClone,indexOfStart);
				}

                // Remove original parent
                var parentPosition = $.inArray(parent,nodes);
                nodes.splice(parentPosition,1);//remove one item at position 

                // Add clone to nodes
                nodes.push(parentClone);
                
				// Split elements parent where element starts

				// do it again one level up with parentClone as delimiter
                oldParent = parent;
				parent = $(parentClone).parent()[0];
				marker = parentClone;

            // Continue until parent is the common ancestor - or until the parent
            // just processed was the common ancestor (this is the case when
            // the start and end marker is in the same element)
            // - or until the parent is the redactor editor
			} while (parent !== commonAncestorContainer && oldParent !== commonAncestorContainer);
			return nodes;
		},
		moveNodesLeft: function(nodeFrom,nodeTo,indexOfDelimiter)
        {
			//insert the clone after the parent, and move all content after the marker
			$(nodeTo).insertAfter(nodeFrom);
			//move all elements from the marker and down to the new node (parent.childNodes.length will decrease as we move the nodes)
			while(nodeFrom.childNodes.length>indexOfDelimiter){
				$(nodeFrom.childNodes[indexOfDelimiter]).appendTo(nodeTo);
			}
		},
		moveNodesRight: function(nodeFrom,nodeTo,indexOfDelimiter)
        {
			$(nodeTo).insertBefore(nodeFrom);
			var childrenToMove = indexOfDelimiter;
			for (var i = 0; i <= childrenToMove; i++) {
				$(nodeFrom.childNodes[0]).appendTo(nodeTo);
			}
		},
		...
		inlineMethods: function(type, attr, value)
		{
			...
			{
				// exec subscript command, since this will intelligently add tags
                // around selected text, no matter what the HTML structure is
                this.document.execCommand('subscript');
                
                // Replace these <sub>-tags
				var tempElements = this.$editor.find('sub');
				$.each(tempElements, $.proxy(function(i, s)
				{
		...
		inlineSetMethods: function(type, s, attr, value)
		{
			...
			{*/
				el = $('<span data-redactor="verified" data-redactor-inlineMethods></span>').append($(s).contents());
				$(s).replaceWith(el);
			/*}*/

			$(el)[type](attr, value);
            
            if (type === 'css') this.inlineSetMethodsInnerElements(type, el, attr, value);
                        
			return el;
		},
        /* 
         * Find inner elements and apply same inline methods as the parent
         * Please be aware, that this overwrites the current methods.
         * Using this function with the type "attr" can thus be inappropiate.
         */
        inlineSetMethodsInnerElements: function(type, $el, attr, value)
        {
            var that = this;
            $el.children('span, strong, b, em, i, code').each(function() {
                $(this)[type](attr, value);
                // call recursively
                that.inlineSetMethodsInnerElements(type, $(this), attr, value);
            });
        },
        elementNodeReplace: function(elem, findElemName, replaceElemName, markWithAttribute) {

            $findElements = $(elem).find(findElemName);
            $findElements.replaceWith(function () {

                var attributes = $(this).prop('attributes');

                // Create new element with content of old element
                $newElement = $('<' + replaceElemName + '/>').html(this.innerHTML);

                // loop through old elements attributes and apply them on the new element
                $.each(attributes, function() {
                    $newElement.attr(this.name, this.value);
                });
                
                if (markWithAttribute) $newElement.attr(markWithAttribute, '');
...
*/
		...
		linkObserver: function(e)
		{
			...
			if (href && href.length > 24) href = href.substring(0,24) + '...';
		...
		getNodes: function(range, tag)
		{
            ...
            if (!this.opts.rangy) {
                ...
                $.each(nodes, $.proxy(function(i,node)
                {
                    ...
                    {
                        if ($.trim(node.textContent) != '' || node.hasChildNodes === true)
                        {
            ...
            // rangy
                ...
                for (var i = 0; i < sel.rangeCount; ++i) {
                    finalnodes = finalnodes.concat( sel.getRangeAt(i).getNodes() );
                }
            }
		...
		linkShow: function()
		{
			...
			var callback = $.proxy(function()
			{
                ...
                // Grab all files in project and generate a list of these for the dropdown -KJI
                var websiteFiles = (function() {
                    options = '';
				...
				if (url.search('mailto:') === 0)
				{
                    ...
                    websiteFiles = '<option value=""></option>\n' + websiteFiles;
				}
				...
				else
				{
                    ...
                    var currentUrl = '<option value="' + turl + '">' + turl + '</option>\n';
                    websiteFiles = websiteFiles.replace(currentUrl, '');
                    websiteFiles = currentUrl + websiteFiles;
				}

                $('#redactor_link_url').html(websiteFiles).ufd();

				if (target === '_blank') $('#redactor_link_blank').prop('checked', true);

                // Tooltip edit -KJI
                if ($(elem).hasClass('imCssLink') && $(elem).is('[onmouseover]')) {
                    var tooltipText = $(elem).attr('onmouseover').match(/text: \'(.*?)\',/i)[1],
                        imgFound = tooltipText.indexOf('img src=');

                    if (imgFound < 0) {
                        $('.redactor_link_title').val(tooltipText);
                        $('<input type="hidden" name="tooltipInUse" id="tooltipInUse" value="1">').insertAfter($('.redactor_link_title'));
                    }
                } else {
                    $('.redactor_link_title').prev().andSelf().remove();
                }
		...
		linkProcess: function()
		{
			...
			var link = '', text = '', target = '', targetBlank = '', tooltip = '';
			...
			if (tab_selected === '1')
			{
				...
				tooltip = $('#redactor_link_url_title').val();
			...
			else if (tab_selected === '2')
			{
				...
				tooltip = $('#redactor_link_mailto_title').val();
			}
			// anchor
			else if (tab_selected === '3')
			{
				...
				tooltip = $('#redactor_link_anchor_title').val();
			}

			text = text.replace(/<|>/g, '');
			this.linkInsert('<a href="' + link + '"' + target + '>' + text + '</a>', $.trim(text), link, targetBlank, tooltip);

		},
		linkInsert: function (a, text, link, target, tooltip)
		{
			this.selectionRestore();
			...
			if (text !== '')
			{
				if (this.insert_link_node)
				{
                    ...
                    // Tooltip edit -KJI
                    if ($('#tooltipInUse').length) {
                        $(this.insert_link_node).attr('onmouseover', $(this.insert_link_node).attr('onmouseover').replace(/text: \'(.*?)\',/gi, 'text: \'' + tooltip + '\','));
                    }
		...
		imageEdit: function(image)
		{
			...
			var callback = $.proxy(function()
			{
				...
				{
                    var currentHref = '<option value="' + $(parent).attr('href') + '">' + $(parent).attr('href') + '</option>\n';
                    
                    // remove current instances of the current href
                    websiteFiles = websiteFiles.replace(currentHref, '');
                    
                    // add current href
                    websiteFiles = currentHref + websiteFiles;
                ...
                $('#redactor_file_link').html(websiteFiles).ufd();
            ...
            // Tooltip edit -KJI
            var $tooltipElem = $el.closest('a.imCssLink');
            if ($tooltipElem.length && $tooltipElem.is('[onmouseover]')) {
                var tooltipText = $tooltipElem.attr('onmouseover').match(/text: \'(.*?)\',/i)[1],
                    imgFound = tooltipText.indexOf('img src=');
                    
                if (imgFound < 0) {
                    $('div#redactor_modal_inner #redactor_file_alt').val(tooltipText);
                    $('<input type="hidden" name="tooltipInUse" id="tooltipInUse" value="1">').insertAfter($('div#redactor_modal_inner #redactor_file_alt'));
                }
		...
		imageSave: function(el)
		{
            ...
            if ($('#tooltipInUse').length) {
                var $tooltipElem = $el.closest('a.imCssLink');
                $tooltipElem.attr('onmouseover', $tooltipElem.attr('onmouseover').replace(/text: \'(.*?)\',/gi, 'text: \'' + $('#redactor_file_alt').val() + '\','));
            }
		...
		modalTemplatesInit: function()
		{
			...
			$.extend( this.opts,
			{
				...
				+ '<section>'
                    ...
                    + '<select id="redactor_file_link" class="redactor_input">'
					+ '</select>'
				...
				+ '<section>'
					+ '<form id="redactorInsertLinkForm" method="post" action="">'
						...
						+ '<div class="redactor_tab" id="redactor_tab1">'
							+ '<label>URL</label>'
                            + '<select id="redactor_link_url" class="redactor_input">'
                            + '</select>'
							...
							+ '<label>' + this.opts.curLang.title + '</label>'
							+ '<input type="text" class="redactor_input redactor_link_title" id="redactor_link_url_title" />'
							//+ '<label><input type="checkbox" id="redactor_link_blank"> ' + this.opts.curLang.link_new_tab + '</label>'
						+ '</div>'
						+ '<div class="redactor_tab" id="redactor_tab2" style="display: none;">'
							...
							+ '<label>' + this.opts.curLang.title + '</label>'
							+ '<input type="text" class="redactor_input redactor_link_title" id="redactor_link_mailto_title" />'
						+ '</div>'
						+ '<div class="redactor_tab" id="redactor_tab3" style="display: none;">'
							...
							+ '<label>' + this.opts.curLang.title + '</label>'
							+ '<input type="text" class="redactor_input redactor_link_title" id="redactor_link_anchor_title" />'
						+ '</div>'
					+ '</form>'
				+ '</section>'
				+ '<footer>'
