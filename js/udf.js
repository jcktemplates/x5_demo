...
$.widget(widgetName, {
	...
	_create: function() { //1.8 init
		...
		this.wrapper = $([
			'<span class="', css.wrapper, ' ', css.hidden, ' ', css.skin, '">',
				...
				'<button type="button" tabindex="-1" class="', css.button, '"></button>',
			//   <select .../> goes here
			'</span>'
		].join(''));
		this.dropdown = $([
	...
	realLooseFocusEvent: function() {
		...
		//
		// AFI - remove tryToSetMaster(), since we need document click to only hide list.
		//
		//this.tryToSetMaster();
	...
	*/
	...
	_populateFromMaster: function() {
		...
		listBuilder.push('<ul style="padding: 4px 10px !important;">');
		var options = this.selectbox.get(0).options;
...
$.extend($.ui.ufd, {
	...
	defaults: { // 1.7 default options location, see below
		...
		zIndexPopup: 50030, // dropdown z-index
